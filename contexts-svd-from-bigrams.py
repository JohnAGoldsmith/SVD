...
def mySVD(this_array, newpath, rank ):

	U,Sigma,V = np.linalg.svd(this_array)       
	np.set_printoptions(precision = 2)

        print >>outfile_testing, "SVD", this_array
	print 
	#print "U", U
	#print "V", V
	#print "Sigma", Sigma
	...
	for eigenvectornumber in range(rank):
 	...
 	outfile.close()
	return U,Sigma,V
def mySVD_contexts(this_array, newpath, rank ):
	...
	for eigenvectornumber in range(rank):
 	...
 	for eigenvectornumber in range(rank):
	    ...
	    print  >>eigenfile, "eigenvector number", eigenvectornumber
	    for i in range(word_size):
		eigenlist.append((index_to_context[i], V[eigenvectornumber,i]))
	    eigenlist.sort(key = lambda x: x[1], reverse = True)
	    j = 0
	    for item in eigenlist:
		print  >>eigenfile, j, index_to_word[item [0][0]], index_to_word[item[0][1]],    '{:f}'.format(item[1])
                j = j + 1
	    print >>eigenfile
	    eigenfile.close()
...
context_array = np.zeros([word_size, context_size])
...
newfolder = outfolder + "/eigenvectors/"
...
for context in topcontexts:
	...
	if  context[1] < MINIMUM_NUMBER_OF_FILLERS:
		continue
	index_to_context[context_number] = thisContext
        context_to_index[thisContext] = context_number
	...
	for filler in fillerList:
                #print filler, context_number
                context_array[ context_number, filler[0] ]  = filler[1]
	...
	print >>outfilecontextfillers
	context_number += 1
        if context_number == context_size:
	    break


newfolder = infolder +  "contexts/"
U,Sigma,V = mySVD_contexts(context_array, newfolder, rank)
